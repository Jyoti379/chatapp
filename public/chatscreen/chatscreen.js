...
/*
 ...
 //groupname
 axios.get("http://localhost:3000/group/getgroup")
 .then(response=>{
  console.log(response);
  let groupdata=response.data.groups;
  const createdGroup=document.getElementById('userbar')
  
  groupdata.forEach(group=>{
    const grouplist=`<li class='chatbar'><a href="../groupScreen.html?groupId=${group.id}">${group.groupname}</a></li>`;
    createdGroup.innerHTML+=grouplist
...
function showChatBar(userdata){
  ...
  userbar.innerHTML=`<button onclick="location.href='../addGroup/chat.html';"> 	&#43 new group</button>`;
  userdata.forEach(user=>{
    const userList=`<li  id="list" class="chatbar">  ${user.name}`;
